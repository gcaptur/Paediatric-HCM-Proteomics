...
col = list(Status = c("0" = "#C5CDD8", "2" = "#D99A3D", "3" = "#8F282B"))
...
# Trajectories of mean clinical biomarker levels with SEM ribbons

DF<-read.table("~/Directory/clinicalmakers.csv", header=TRUE, sep=",") 
DF=read.table("~/Directory/clinicalmakers.csv", header=TRUE, sep=",")
par(mar = c(1,1,1,1))

ggplot(DF, aes(DF$X, DF$Y)) + theme_bw() +
  geom_ribbon(aes(x=DF$X, ymin=DF$n, ymax=DF$p), fill = "#76b5c5", alpha=0.2)+                                   
  geom_line(color = "#76b5c5", size = 0.8) +
  facet_wrap(. ~ Facets, scales = "free_y", nrow=1)

# Trajectories of mean proteomics biomarker levels with SEM ribbons

DF<-read.table("~/Directory/proteomicsmarkers.csv", header=TRUE, sep=",") 
DF=read.table("~/Directory/proteomicsmarkers.csv", header=TRUE, sep=",")
par(mar = c(1,1,1,1))

the_plot <- ggplot(DF, aes(DF$X, DF$Y)) + theme_bw() +
  geom_ribbon(aes(x=DF$X, ymin=DF$n, ymax=DF$p), fill = "indianred4", alpha=0.2)+                                   
  geom_line(color = "indianred4", size = 0.8) +
  facet_wrap(. ~ Facets, scales = "free_y", nrow=8)  
the_plot 

# Similarity measures (examplar analyte and clinical marker)

g1<-read.table("~/Directory/simi.csv", header=TRUE, sep=",")
g1=read.table("~/Directory/simi.csv", header=TRUE, sep=",")
 
	path1<-as.matrix(g1$ELADZ, g1$scale)		
	path2<-as.matrix(g1$A2AP, g1$scale)
